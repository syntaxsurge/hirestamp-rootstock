...
/// @title HireStamp DID Registry
/// @notice Allows each address to mint one `did:rsk:0x…`, update its document pointer,
...
contract DIDRegistry is AccessControl {
    ...
    function _deriveDID(address owner) private pure returns (string memory) {
        return string.concat("did:rsk:", Strings.toHexString(uint160(owner), 20));
    }
