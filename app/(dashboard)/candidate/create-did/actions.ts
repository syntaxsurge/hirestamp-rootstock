...
/**
 * Persist a freshly created Rootstock DID for the caller’s team.
 *
...
export const createDidAction = validatedActionWithUser(
  z.object({
    did: z
      .string()
      .regex(/^did:rsk:0x[0-9a-fA-F]{40}$/, 'Invalid Rootstock DID (expected did:rsk:0x…)'),
  }),
  async ({ did }, _formData, user) => {
    ...
    /* ------------------------------------------------------------ */
    const expectedDid = `did:rsk:${user.walletAddress}`
