...
import { createRootstockDID } from '@/lib/contracts/hirestamp'
...
const schema = z.object({
  ...
  did: z
    ...
    .trim()
    .optional()
    .refine((v) => !v || /^did:rsk:0x[0-9a-fA-F]{40}$/.test(v), {
      message: 'Invalid Rootstock DID (expected did:rsk:0x…)',
    }),
})
...
export const upsertPlatformDidAction = validatedActionWithUser(
  ...
  async ({ did }, _formData, user) => {
    ...
    if (!newDid) {
      ...
      try {
        /* Attempt fresh mint -------------------------------------------- */
        const { did: generatedDid } = await createRootstockDID({ signer: platformSigner })
        newDid = generatedDid
      } catch (err: any) {
        const msg =
        ...
        if (!alreadyExists) {
          return { error: `Failed to create Rootstock DID: ${msg}` }
        }
